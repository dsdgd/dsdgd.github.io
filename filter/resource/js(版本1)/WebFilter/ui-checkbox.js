...
define(function (require, exports, module){
	...
	function ui(oParent, oChild, oBtn, oSelect, Json, url){
		...
		$(oBtn).click(function (){
			...
			//这里需要提交一次Ajax
			var sUrl = require('./data-getKeyWord.js').init(Json);
			sUrl = encodeURI(sUrl);
			require('./data-ajax.js').init(url+'?' + sUrl);
		});
		...
		$(oSelect).click(function (){
			...
			if( $(this).prop('checked') ){
				...
				$(oChild).find('dd input[type=checkbox]').prop({checked:true});
				
			}else{
				
				$(this).next('span').html('全选');
				$(oChild).find('dd input[type=checkbox]').prop({checked:false});
			}
