		...
		jQuery(document).ready(function (){
			...
			$.ajax({
				...
				success: function(Json){
					...
					//解析筛选数据
					var oJson = require('./data-processing.js').initResult(Json);
					
					//生成筛选数据UI
					require('./ui-screening.js').ui(oJson, url, aOrderBy);
					require('./ui-toggle.js').ui();
					
					//发送表头数据请求
					var sUrl = url+'?tName=' + oJson['title']['TableName'] + '&filter=&IsAjax=true&Labels=商品综合信息列表Table';
					sUrl = encodeURI(sUrl);
					require('./data-ajax.js').init(sUrl);
					
					//重置UI
					require('./ui-reset.js').init('#ZlgReset', sUrl);
