...
define(function(require, exports, module) {
    function init(Json, bFirst, config) {//Json：数据, bFirst：true为筛选参数，false为表格数据
        ...
        var aTmpIndex = [];//暂时存放对应title的产品信息下标数据
        var iPartNo = 0;//存放产品型号下标
        var iItemID = 0;//存放产品ID下标
        var iBest = true;//存放推荐产品下标
        var aProduct = [];//存放产品链接
        var aInfo = [];//存放产品信息

       for(var i = 0; i < iLen0; i++){//查找下标
        ...
        for(var i = 0; i < iLen1; i++){//获取产品链接、产品信息
        ...
        for(var i = 0; i < iLen0; i++){ //创建数据分组
            var oTmp = {};
            oTmp[ Json['Param'][i] ]  = {'str':[], 'num':[]}
            aData.push(oTmp);
        };
        ...
        for(var i = 0; i < iLen3; i++){//排序、去重
            for(var j in aData[i]){
                aData[i][j]['num'].sort(function (a,b){return a-b});
                aData[i][j]['num'] = aData[i][j]['num'].unique();
                aData[i][j]['str'].sort(function (a,b){return a.localeCompare(b)});
