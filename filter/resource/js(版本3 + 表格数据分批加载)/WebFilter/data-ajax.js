...
define(function (require, exports, module){
	...
	function init(URL, config){
		...
		$.ajax({
			...
			success: function(result){
				...
				result = eval('('+result+')');
				var oJson = require('./data-processing.js').init(result, false, config);
				
				var sTmp1 = '', sTmp2 = '';//暂存，拼接字符串
	...
	function page(Json, URL, config){
		...
		var iMaxCount = Json['Count'];	//分页总数
		var iPageSize = config['pagesize'];	//单页数据量
		var iNowSize = Json['Rows'].length; //当前记录量
		var iNowIndex = Json['Index'];	//当前页码
		var iMaxNum = 10; //最大页码按钮量
		...
		for(var i = 0; i < iMaxCount+4; i++){//拼接页面html
		...
		$('#ZlgPageList').html(sBtn);//生成页码

		$('#ZlgPageList').children().bind('click', function (){	//分页点击事件
